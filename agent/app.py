"""
HTTP entrypoint - the same contract on every deployment target.
...
saf3ai_setup.start()  # must run before any LLM client is created

from fastapi import FastAPI  # noqa: E402
from fastapi.responses import JSONResponse  # noqa: E402
from pydantic import BaseModel  # noqa: E402

import agent  # noqa: E402  (creates the LLM client)

app = FastAPI(title="Saf3AI sample agent")
...
@app.get("/healthz")
def healthz():
    return {"status": "ok", "provider": agent.provider.name, "model": agent.provider.model}
