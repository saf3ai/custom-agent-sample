...
import agent  # noqa: E402  (initialises Saf3AI at import, then builds the ADK agent)

from fastapi import FastAPI  # noqa: E402
from fastapi.responses import JSONResponse  # noqa: E402
from pydantic import BaseModel  # noqa: E402

app = FastAPI(title="Saf3AI sample agent (Google ADK)")
...
@app.get("/healthz")
def healthz():
    return {"status": "ok", "framework": "google-adk", "model": agent.MODEL}


@app.post("/chat")
def chat(req: ChatRequest):
    conversation_id = req.conversation_id or str(uuid.uuid4())
    handler = functools.partial(agent.answer, user_id=req.user_id or "anonymous",
                                session_id=saf3ai_setup.adk_session_id(conversation_id))
    try:
