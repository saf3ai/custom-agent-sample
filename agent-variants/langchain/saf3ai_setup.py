...
Everything Saf3AI-specific in this agent lives in this one file.
  ...
  langchain_callbacks() - the SDK's LangChain callback handler, for the run config.
  traced_tool()         - decorator that records each tool call as a span.

Differs from agent/saf3ai_setup.py (LangChain only):
  - adds langchain_callbacks(): records a verdict on each model call for
    visibility. Blocking is done by run_turn(), before the agent runs.
  - adds traced_tool(): a span per tool call.
...
def langchain_callbacks() -> list:
    """Callback handlers to pass as config={"callbacks": ...}. Call after start().

    Detection only: on every model call the handler scans the full model input
    (system prompt, history and tool results included) and records the verdict
    on the model-call span. The final reply is already scanned by run_turn().
    """
    global _lc_handler
    if not SCANNER:
        return []
    if _lc_handler is None:
        ...
        _lc_handler = create_security_callback(api_endpoint=SCANNER, api_key=API_KEY,
                                               agent_identifier=AGENT_ID, scan_responses=False)
    return [_lc_handler] if _lc_handler else []
