...
Differs from agent/saf3ai_setup.py (OpenAI Agents SDK only): adds traced_tool().
This agent uses the SDK's framework-agnostic "custom" integration: run_turn()
gives the turn trace, the scans and the blocking, and
traced_tool() adds the tool calls.

Tested with saf3ai-sdk 0.2.4 (PyPI) and openai-agents 0.22.
"""
import functools
import json
import logging
